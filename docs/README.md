...
- `architecture.svg` - the data flow of PRD §7.2 as built (below). Drawn by `scripts/docs/architecture-diagram.mjs`; edit the text there and rerun `node scripts/docs/architecture-diagram.mjs`. The box text was checked against the code on 18 September 2026, so re-check it when a step changes.
...
- `deployment.md` - running it on a host: the single-server shape, and Vercel (static web app + the API as one function) with the Redis session store.
...
| 4 Chunk & keep | `api-server/src/analysis/chunks.ts`, `lib/rules/src/clause-label.ts`, `api-server/src/sessions/` (`store.ts` the contract, `memory-store.ts` TTL + sweeper, `redis-store.ts` + `sealed.ts` the shared, encrypted store) |
| 5 Select evidence | `lib/rules/src/registry.ts` + `engine.ts`, `api-server/src/analysis/document-map.ts`, `dates.ts`, `parties.ts`, `review-prompts.ts`, `compare/`; for a typed question, `ask.ts` over `lib/grounding/src/retrieval.ts` (BM25) |
...
| 7 Render | `clausecompass/src/pages/document-map.tsx`, `review-prompts.tsx`, `ask.tsx` + `features/ask/`, `compare.tsx`; `features/grounding/` (`resolve-claim.ts`, `source-card.tsx`, `format-location.ts`); `features/display/`, `features/speech/` |
...
Built and tested but not wired to any screen yet (named in the diagram's notes, not drawn as steps): chunk-level instruction flags (`lib/rules/src/untrusted.ts`), the stateless `POST /api/documents/extract` route.
