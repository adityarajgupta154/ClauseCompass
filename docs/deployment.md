...
`SESSION_STORE=redis` keeps each session as one hash, `session:<id>`, in a Redis database reached over Upstash's REST API (`src/sessions/redis-store.ts`, `upstash-rest.ts`; no Redis client package). What the database holds:
...
Rotating `SESSION_STORE_KEY` retires every live session at once, which is the intended response to a suspected leak of the key or the database token.
...
1. **Create the Redis database.** In the Vercel project, Storage → Marketplace → Upstash → Redis (or create one in the Upstash console). Use a fresh database for this app alone. The Vercel integration injects `KV_REST_API_URL` and `KV_REST_API_TOKEN` into the project's environment; the API reads those names when `SESSION_STORE_URL` / `SESSION_STORE_TOKEN` are not set by hand.
...
5. **Deploy**, then check `https://<domain>/api/healthz` (200), sign in, upload a PDF from `samples/` and open the document map, and press "Delete my document now". The PDF matters: it proves the tracer shipped pdf.js with the function. A `store-unavailable` error on upload means the database variables are missing or wrong; the function log shows the configuration problem by name.
